...
El modelo `User` está diseñado para almacenar información detallada sobre los usuarios de la plataforma, incluyendo datos personales, preferencias, roles y actividad. Este documento proporciona una guía completa sobre la estructura y el uso del modelo `User`.
...
| `address`                            | `AddressSchema`    | Dirección del usuario. Ver [Address](/docs/database/models/secondary-models/address).        |
...
| `properties`                         | `Array<ObjectId>`  | Arreglo de identificadores de las propiedades asociadas al usuario. Ver [Property](/docs/database/models/property). |
| `notifications`                      | `Array<NotificationSchema>` | Arreglo de notificaciones asociadas al usuario. Ver [Notification](/docs/database/models/secondary-models/notification). |

## Roles de Usuario

Todos los roles de usuario utilizan el mismo modelo de datos, pero los campos que no corresponden a un tipo de usuario específico serán **nulos**. Por ejemplo, un usuario con rol 2 (Guest) tendrá los campos `tax_document` y `properties`, pero estarán vacíos o serán nulos.
